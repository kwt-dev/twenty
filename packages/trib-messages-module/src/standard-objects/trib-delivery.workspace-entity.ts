...
@WorkspaceEntity({
  ...
  labelSingular: 'TRIB Delivery',
  labelPlural: 'TRIB Deliveries',
  description:
    'Message delivery tracking record with comprehensive status and error tracking',
  icon: 'IconTruck',
  shortcut: 'D',
...
export class TribDeliveryWorkspaceEntity extends BaseWorkspaceEntity {
   ...
   * ONE_TO_ONE relationship with TribMessage
   * Each delivery record tracks one message
   */
  @WorkspaceField({
    standardId: DELIVERY_FIELD_IDS.messageId,
    type: 'RELATION',
    label: 'Message',
    description: 'Message being tracked for delivery',
    icon: 'IconMessage',
  })
  @WorkspaceRelation({
    relationType: 'ONE_TO_ONE',
    type: () => 'TribMessageWorkspaceEntity',
    inverseSideFieldKey: 'delivery',
    onDelete: 'CASCADE', // When message is deleted, delivery is also deleted
  })
  @WorkspaceJoinColumn('messageId')
  ...
  @WorkspaceField({
    ...
    type: 'UUID',
    label: 'Message ID',
    description: 'ID of the message being delivered',
    icon: 'IconMessage',
  })
  messageId: string;
  ...
  @WorkspaceField({
    ...
    type: 'SELECT',
    label: 'Status',
    description: 'Comprehensive delivery status',
    icon: 'IconCheck',
    options: [
      { value: TribDeliveryStatus.QUEUED, label: 'Queued', color: 'blue' },
      { value: TribDeliveryStatus.SENDING, label: 'Sending', color: 'yellow' },
      { value: TribDeliveryStatus.SENT, label: 'Sent', color: 'green' },
      {
        value: TribDeliveryStatus.DELIVERED,
        label: 'Delivered',
        color: 'green',
      },
      { value: TribDeliveryStatus.FAILED, label: 'Failed', color: 'red' },
      {
        value: TribDeliveryStatus.UNDELIVERED,
        label: 'Undelivered',
        color: 'orange',
      },
      { value: TribDeliveryStatus.CANCELED, label: 'Canceled', color: 'gray' },
      { value: TribDeliveryStatus.ACCEPTED, label: 'Accepted', color: 'blue' },
      {
        value: TribDeliveryStatus.RECEIVING,
        label: 'Receiving',
        color: 'purple',
      },
      { value: TribDeliveryStatus.RECEIVED, label: 'Received', color: 'green' },
    ],
  ...
  @WorkspaceField({
    ...
    description: 'When delivery status was last updated',
    icon: 'IconCalendar',
  })
  @WorkspaceIsNullable()
  timestamp: Date | null;
  ...
  @WorkspaceField({
    ...
    type: 'TEXT',
    label: 'Provider',
    description: 'Message delivery provider (TWILIO, AWS_SNS, etc.)',
    icon: 'IconCloud',
  })
  provider: string;
  ...
  @WorkspaceField({
    ...
    type: 'TEXT',
    label: 'Error Code',
    description: 'Provider-specific error code for failed deliveries',
  ...
  @WorkspaceField({
    ...
    type: 'TEXT',
    label: 'Error Message',
    description: 'Detailed error message from provider',
    icon: 'IconAlertTriangle',
  })
  @WorkspaceIsNullable()
  ...
  @WorkspaceField({
    ...
    type: 'NUMBER',
    label: 'Cost',
    description: 'Delivery cost in cents (USD)',
    icon: 'IconCoin',
  })
  @WorkspaceIsNullable()
  cost: number | null;
  ...
  @WorkspaceField({
    ...
    icon: 'IconWebhook',
    options: [
      { value: TribCallbackStatus.PENDING, label: 'Pending', color: 'blue' },
      {
      ...
      { value: TribCallbackStatus.FAILED, label: 'Failed', color: 'red' },
      {
  ...
  @WorkspaceField({
    ...
    description: 'Provider delivery ID (e.g., Twilio SID)',
  ...
  @WorkspaceField({
    ...
    type: 'RAW_JSON',
    label: 'Metadata',
    description: 'Provider-specific metadata and debugging information',
    icon: 'IconCode',
  })
  @WorkspaceIsNullable()
